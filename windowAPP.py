...
# 시리얼 포트 설정
SERIAL_PORT = 'COM5'  # 시리얼 포트 이름 (로봇이 연결된 포트에 맞게 수정)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def video_pred(self):  # 비디오 감지
        ...
        if not ret:
            ...
            if len(detections) > 0:
                sorted_detections = sorted(detections, key=lambda x: x[4], reverse=True)[:1]  # confidence 값 기준으로 정렬 후 상위 2개 선택
